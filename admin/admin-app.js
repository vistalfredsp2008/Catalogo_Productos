...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Vinculación para exportar config.json
  const btnExpConfig = document.getElementById("btn-exportar-config");
  if (btnExpConfig) {
    btnExpConfig.addEventListener("click", exportarConfiguracionJSON);
  }
...
function exportarConfiguracionJSON() {
  const contacto = JSON.parse(localStorage.getItem("mi_config_contacto_admin")) || {};
  const apariencia = JSON.parse(localStorage.getItem("mi_config_apariencia_admin")) || {};
  ...
  const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(configCompleta, null, 2));
  const downloadAnchor = document.createElement("a");
  downloadAnchor.setAttribute("href", dataStr);
